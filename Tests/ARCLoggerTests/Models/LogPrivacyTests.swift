...
struct LogPrivacyTests {
    ...
    @Test("Privacy levels are distinct") func privacyLevelsAreDistinct() {
    ...
    @Test("Same privacy levels are equal") func sameLevelsAreEqual() {
...
struct LogValueTests {
    ...
    @Test("LogValue initializes with value and privacy") func initializesCorrectly() {
    ...
    @Test("Public factory creates public log value") func publicFactoryWorks() {
        let logValue = LogValue.public("visible")
        #expect(logValue.value == "visible")
        #expect(logValue.privacy == .public)
    }

    @Test("Private factory creates private log value") func privateFactoryWorks() {
        let logValue = LogValue.private("hidden")
        #expect(logValue.value == "hidden")
        #expect(logValue.privacy == .private)
    }

    @Test("Sensitive factory creates sensitive log value") func sensitiveFactoryWorks() {
    ...
    @Test("Description returns raw value") func descriptionReturnsValue() {
    ...
    @Test("Public values are never redacted in development") func publicNotRedactedInDev() {
        let logValue = LogValue.public("visible")
        #expect(logValue.redacted(isProduction: false) == "visible")
    }

    @Test("Public values are never redacted in production") func publicNotRedactedInProd() {
        let logValue = LogValue.public("visible")
        #expect(logValue.redacted(isProduction: true) == "visible")
    }

    @Test("Private values are visible in development") func privateVisibleInDev() {
        let logValue = LogValue.private("hidden")
        #expect(logValue.redacted(isProduction: false) == "hidden")
    }

    @Test("Private values are redacted in production") func privateRedactedInProd() {
        let logValue = LogValue.private("hidden")
        #expect(logValue.redacted(isProduction: true) == "<private>")
    }

    @Test("Sensitive values are always redacted in development") func sensitiveRedactedInDev() {
        let logValue = LogValue.sensitive("secret")
        #expect(logValue.redacted(isProduction: false) == "<sensitive>")
    }

    @Test("Sensitive values are always redacted in production") func sensitiveRedactedInProd() {
        let logValue = LogValue.sensitive("secret")
