...
struct LogEntryTests {
    ...
    @Test("LogEntry initializes with all parameters") func initializesWithAllParams() {
        let timestamp = Date()
    ...
    @Test("LogEntry initializes with default values") func initializesWithDefaults() {
        ...
        #expect(entry.subsystem.isEmpty)
    ...
    @Test("LogEntry preserves custom subsystem and category") func customSubsystemAndCategory() {
        let entry = LogEntry(message: "Test",
                             level: .info,
    ...
    @Test("Message can be empty") func emptyMessageAllowed() {
        let entry = LogEntry(message: "", level: .info)
        #expect(entry.message.isEmpty)
    }

    @Test("Message preserves whitespace") func whitespacePreserved() {
        let entry = LogEntry(message: "  spaced  message  ", level: .info)
        #expect(entry.message == "  spaced  message  ")
    }

    @Test("Message preserves newlines") func newlinesPreserved() {
    ...
    @Test("All log levels can be used", arguments: LogLevel.allCases) func allLevelsWork(level: LogLevel) {
    ...
    @Test("Metadata can be empty") func emptyMetadata() {
        let entry = LogEntry(message: "Test", level: .info, metadata: [:])
        #expect(entry.metadata.isEmpty)
    }

    @Test("Metadata with multiple entries") func multipleMetadataEntries() {
        let metadata: [String: LogValue] = ["key1": .public("value1"),
                                            "key2": .private("value2"),
    ...
    @Test("Timestamp is set correctly") func timestampIsSet() {
        let before = Date()
    ...
    @Test("Custom timestamp is preserved") func customTimestampPreserved() {
    ...
    @Test("fileName extracts last path component") func fileNameExtractsLastComponent() {
        let entry = LogEntry(message: "Test",
                             level: .info,
    ...
    @Test("fileName handles simple file names") func fileNameHandlesSimple() {
        let entry = LogEntry(message: "Test",
                             level: .info,
                             file: "File.swift",
                             function: "test()",
    ...
    @Test("fileName handles paths with spaces") func fileNameHandlesSpaces() {
        let entry = LogEntry(message: "Test",
                             level: .info,
    ...
    @Test("Source location captures current file") func capturesCurrentFile() {
        let entry = LogEntry(message: "Test", level: .info)
        #expect(entry.file.contains("LogEntryTests.swift"))
    }

    @Test("Line number is captured correctly") func lineNumberCaptured() {
        let entry = LogEntry(message: "Test", level: .info)
        #expect(entry.line > 0)
    }
}
